...
/**
 * Used to validate that the Product Image control is displayed in the UI when a specific product is scanned.
 */
public class UT15 {
    ...
    public void ut15() throws InterruptedException
    {
        mActivityScenarioRule.getScenario().onActivity(activity ->
        {
            activity.runOnUiThread(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    //that is not contained in the Barcode Lookup API which will not return a product image.
        ...
        onView(withId(R.id.ProductImageView)).check(matches(not(isDisplayed())));
